...
<table class="responstable table">
  ...
  <?php 
	  ...
	  $loc = get_NearestLocation($BusList[$i]->Latitude,
	  	$BusList[$i]->Longitude);
	  echo '<td>'.$loc->name.'</td>';
	  $Loc1 = get_LocationDetails($BusList[$i]->Location_ID1)->name;
	  $Loc2 = get_LocationDetails($BusList[$i]->Location_ID2)->name;
	  $txt;
	  if($BusList[$i]->Direction == 1){
	  	$txt = $Loc1.'-'.$Loc2;
	  }
	  else{
	  	$txt = $Loc2.'-'.$Loc1;	
	  ...
	  echo '<td>'.$txt.'</td>';
	  echo '<td><a href="BusHistory.php?RegNo='.$BusList[$i]->RegNo.'"><div class="containerbtn">Go</div></a></td>';
	  echo '</tr>';
	}
  ?>
